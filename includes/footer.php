<div class="container">
    <div class="row">
        <div class="col-3">
            <span>Sobre</span>
            <ul>
                <a href="quem-somos"><li>Quem somos</li></a>
                <a href="servicos"><li>Serviços</li></a>
                <a href="perguntas-frequentes"><li>Perguntas Frequentes</li></a>
                <a href="termos-e-condicoes"><li>Termos e Condições</li></a>
            </ul>
        </div>

        <div class="col-3">
            <span>Aprendizado</span>
            <ul>
                <a href="ajuda"><li>Ajuda</li></a>
                <a href="dicas-para-negocios"><li>Dicas para negócios</li></a>
                <a href="servicos-locais"><li>Serviços Locais</li></a>
                <a href="depoimentos"><li>Depoimentos</li></a>
            </ul>
        </div>

        <div class="col-3">
            <span>Redes Sociais</span>
            <ul>
                <a href="https://facebook.com/localfybrasil" target="_blank"><li><i class="bi-facebook" style="font-size: 20px;"></i> Facebook</li></a>
                <a href="https://linkedin.com/localfybrasil" target="_blank"><li><i class="bi-linkedin" style="font-size: 20px;"></i> Linkedin</li></a>
                <a href="https://twitter.com/localfybrasil" target="_blank"><li><i class="bi-twitter" style="font-size: 20px;"></i> Twitter</li></a>
                <a href="https://instagram/localfybrasil" target="_blank"><li><i class="bi-instagram" style="font-size: 20px;"></i> Instagram</li></a>
            </ul>
        </div>
    </div>
    <hr>
    <p>Todos os direitos reservados - Localfy</p>
</div>

<div class="cookie" id="cookieConsent">
    <p>Precisamos utilizar cookies para coletar informações sobre 
    sua navegação em nosso website e melhorar sua experiência. Visite 
    nossa <a href="">Página de Cookies</a> para mais informações. Você aceita o 
    uso de cookies?</p>
    <div class="buttons">
        <button onclick="aceitarCookies()">Recuso os cookies</button>
        <button onclick="aceitarCookies()">Aceito os cookies</button>
    </div>
</div>